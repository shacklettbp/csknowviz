    ...
    const EditorCam &cam, float aspect)
{
    ...
    if (cam.perspective) {
        ...
        proj = makePerspectiveMatrix(tan_hfov, aspect, 1.f,
                                     100000.f);
    } else {
        proj = makeOrthographicMatrix(cam.orthoHeight, aspect, 0.1f,
                                     100000.f);
    }
